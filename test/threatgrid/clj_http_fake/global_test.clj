(ns ^:eftest/synchronized threatgrid.clj-http-fake.global-test
  (:require [clojure.test :refer :all]
            [clj-http.client :as http]
            [clj-http.core :as core]
            [clj-http.util :as util]
            [threatgrid.clj-http-fake.global :as global :refer :all])
...
(deftest falls-through-to-real-request-method-if-no-matching-route
  (with-redefs [clj-http.core/request
    ...
    (#'global/initialize-request-hook)
    (with-fake-routes
...
(defn deref-timely [dr]
  (let [res (deref dr 5000 ::fail)]
    (assert (not= ::fail res) "deref timed out")
    res))
